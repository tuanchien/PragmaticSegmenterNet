...
{
    private static readonly IReadOnlyList<string> RomanNumerals =
    [
        "i",
        "ii",
        "iii",
    ...
    private static readonly IReadOnlyList<string> LatinNumerals =
    [
    ...
    private static string ScanLists(string text, Regex regex1, Regex regex2, string replacement, bool strip = false)
    {
        ...
        {
            list.Add(int.Parse(matches[i].Value.Trim(), System.Globalization.CultureInfo.InvariantCulture));
        }
    ...
    private static string substitute_found_list_items(string text, Regex regex, int item, bool strip, string replacement)
    {
        var result = regex.Replace(text, match =>
        {
            ...
            var itemString = item.ToString(System.Globalization.CultureInfo.InvariantCulture);
    ...
    private static string ReplaceItemsFromAlphabetArray(Regex regex, string text, bool hasParentheses = false, bool isRomanNumeral = false)
    {
        ...
        for (int i = 0; i < matches.Count; i++)
        {
            ...
            listArray.Add(match.Value.ToLowerInvariant());
        }
    ...
    private static string ReplaceAlphabetListWithParentheses(string text, string item)
    {
        var result = ExtractAlphabeticalListLettersRegex.Replace(text, match =>
        {
            ...
            {
                var tidied = OpenParenthesis.Replace(match.Value.ToLowerInvariant(), string.Empty);
            ...
            var lowercaseMatch = string.Equals(item, match.Value, StringComparison.OrdinalIgnoreCase);
            if (lowercaseMatch)
            {
    ...
    private static string? GetRubyIndexed(List<string> list, int index)
    {
        if (index < 0)
        {
